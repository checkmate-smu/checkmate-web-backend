...
이 문서는 CheckMate 백엔드의 도메인 모델을 DDD 어휘(Aggregate / Entity / Value Object / Domain Event / Bounded Context)로 정리한 것이다. 코드와 ERD가 source of truth이며, 본 문서는 **개념 분류 + invariant + 라이프사이클**을 박제한다.

---

## 1. Bounded Context (개념 경계)

CheckMate는 단일 모듈(`com.checkmate.web`)이지만 도메인 책임은 4개로 나뉜다. 향후 Modulith 도입 시 경계 후보.
...
> **현재 패키지 구조는 layer-based** (`controller/service/repository/entity`)이므로 위 경계는 개념적이며 코드 레벨로 강제되지 않는다. Sprint 3+ 도메인 분리 결정 시 `com.checkmate.web.{analysis,verification,account,audit}.*` 재구조화 후보.
